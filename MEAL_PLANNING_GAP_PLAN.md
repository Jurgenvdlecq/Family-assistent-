...
| **Risico's** | Laag/middel — vooral UX-risico (niet te veel keuzestress toevoegen aan een scherm dat de productrichting juist rustig en met één hoofdactie wil houden, zie PROJECT_BLUEPRINT.md Fase 12). Moet zorgvuldig samen met wens 6 ontworpen worden. |
...
| **Risico's** | Een echte "kies zelf 3 losse bouwstenen"-versie raakt de quantity-/package-engine (PROJECT_BLUEPRINT.md Fase 3, expliciet buiten scope van deze opdracht: "vervang de verpakkings-/hoeveelhedenengine niet") en is dus bewust niet de voorgestelde eerste stap. |
...
| | |
|---|---|
| **Bestaande bouwstenen** | `ProductProvider`-enum (vandaag alleen `PICNIC`); `MatchStatus` kent al `NOT_FOUND`/`UNAVAILABLE`; de controlepagina heeft al een "Niet gevonden"-sectie met "zonder product doorgaan" (PROJECT_BLUEPRINT.md Fase 6, al gebouwd volgens `SYSTEM_AUDIT.md`); `addManualProduct` (`manualProductActions.ts`) voegt een eenmalig product toe, maar altijd nog gekoppeld aan een gevonden Picnic-`externalRef`/naam via zoeken — niet bedoeld voor iets dat principieel niet in Picnic te vinden is. |
...
7. **3 suggesties per avond (5)** — vereist eerst punt 6 idealiter niet,
   ...
   scherm hebben, PROJECT_BLUEPRINT.md Fase 12) voordat er gebouwd wordt.
