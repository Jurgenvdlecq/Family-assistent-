...
- **Volgende WP:** `MealPlanEntry`-context voor beter leren uit stil accepteren en handmatige wijzigingen (zie `DATAMODEL_AUDIT.md` punt 6) — uitbreiden met `source` (AUTO/MANUAL/ASSISTANT/REGENERATED), `status` (PROPOSED/ACCEPTED/REPLACED), `reason`, `score`, `confidenceLevel`, `replacedFromRecipeVariantId`. Dit maakt "stilzwijgend bevestigen" (PRODUCT_VISION.md: "Stilte is feedback") onderscheidbaar van een expliciete keuze, en legt de basis om een vaste daggewoonte (WP51) of losse maaltijd (WP49) later te herkennen als "hier is al eens expliciet voor gekozen".
