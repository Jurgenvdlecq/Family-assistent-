...
- Verder geen vastgelegde "volgende WP" — kies in overleg met de gebruiker, of gebruik `MealPlanEntry.status`/`reason` (WP52) nu als basis om daadwerkelijk te leren van stil geaccepteerde weekplanningen (bijv. via `LearnedPattern`/`LearningPrompt`, WP23's fundament).
